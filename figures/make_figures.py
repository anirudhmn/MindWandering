...
def figure3():
    ...
    # (b) duration, across-word vs within-token
    ...
    # (c) neural frequency kernel split by state
...
# =============================================================== FIGURE 4
def figure4():
    fig = plt.figure(figsize=(13.2 * CM, 10.6 * CM))
    gs = fig.add_gridspec(2, 2, wspace=0.62, hspace=0.62, left=0.145, right=0.985, top=0.91, bottom=0.085)
    ...
    ax = fig.add_subplot(gs[1, 1]); panel_label(ax, "d", dx=-0.22, dy=1.17)
...
def figure5():
    ...
    fig = plt.figure(figsize=(18 * CM, 6.6 * CM))
    gs = fig.add_gridspec(1, 3, width_ratios=[1, 1, 1.42], wspace=0.68,
                          left=0.105, right=0.985, top=0.80, bottom=0.235)
    ...
    ax = fig.add_subplot(gs[0, 2]); panel_label(ax, "c", dx=-0.36, dy=1.16)
    rows = [
